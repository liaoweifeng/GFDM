...
% Plot the results
hold off;
semilogy(snr, seroQAM'/(p.B*p.M*p.K),'-ro');
hold on;
semilogy(snr, serMF'/(p.B*p.M*p.K),'-g*');
hold on;
semilogy(snr,  serZF'/(p.B*p.M*p.K),'-bd');
hold on;
semilogy(snr,  serMMSE'/(p.B*p.M*p.K),'-k^');
axis([0 10 10e-4 inf])
legend({'QAM theoretical value','MF receiver','ZF receiver','MMSE receiver'})
hold on;
%semilogy(snr, calc_ser_ofdm_awgn(p.mu, snr), 'b--');
%semilogy(snr, calc_ser_awgn(p,'MF', snr), 'g--');
%semilogy(snr, calc_ser_awgn(p,'ZF', snr), 'r--');
grid;
%legend({'Matched filter', 'Zero forcing', 'MMSE'});
xlabel('SNR/dB')
ylabel('SER')
title('Roll-off factor 0.5')
end
